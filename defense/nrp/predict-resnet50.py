...
# 使用 torchvision 官方 ImageNet 预训练权重，模型改为 ResNet50
source_model = models.resnet50(
    weights=models.ResNet50_Weights.IMAGENET1K_V1  # 官方权重
).to(device).eval()
...
# ResNet50 专用图像预处理函数
# 输出为 [1, 3, 224, 224] 的张量，已移动到指定 device

def predict_resnet50(img_path, device=device):
    transform_resnet50 = transforms.Compose([
        transforms.Resize(256),      # 短边 256
    ...
    x = transform_resnet50(img).unsqueeze(0).to(device)
    return x
...
with torch.no_grad():
    for _, row in tqdm(label_df.iterrows(), desc='Predicting', total=total_num):
        ...
        try:
            # 使用 ResNet50 专用预处理
            x = predict_resnet50(img_path, device=device)
